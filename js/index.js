...
var handleClickA = function(event) {
  ...
  var matchAslice = matchA.slice(0,-4);
    ...
    for (var i = 0;i<productNames.length; i++){
  if(allProducts[i].name === matchAslice){
    console.log("match again here. get lunch!!!");
    console.log(allProducts[i]);
    allProducts[i].tally++;
    // nameOcc.unshift(allProducts[i].name);
...
// var matchB = document.getElementsByTagName('input')[1].src.slice(0,4)
console.log(matchB);
var matchBslice = matchB.slice(0,-4);
console.log(matchBslice);

  for (var i = 0;i<productNames.length; i++){
if(allProducts[i].name === matchBslice) {
  console.log("match again here. get lunch!!!");
  console.log(allProducts[i]);
  allProducts[i].tally++;
  console.log(allProducts[i].tally);
...
var handleClickC = function(event) {
  ...
  var matchCslice = matchC.slice(0,-4);

    for (var i = 0;i<productNames.length; i++){
  if(allProducts[i].name === matchCslice){
    console.log("match again here. get lunch!!");
    // console.log(allProducts[i]);
    allProducts[i].tally++;
    // console.log(allProducts[i].tally);
...
function equalityCheck(){
  ...
  {console.log("all are different");
    ...
    if(allProducts[i].path.substring(7) === srcA){
      ...
      userViews2.unshift(allProducts[i].views)
        nameOcc.unshift(allProducts[i].name);
    }
    }
...
var getResList = document.getElementsByTagName('ul')[0];
for (var i = 0; i<allProducts.length; i++){
  var createLi = document.createElement('li');
  var textLi = document.createTextNode(allProducts[i].name + ": Views: " + allProducts[i].views + " Times chosen: " + allProducts[i].tally);
  createLi.appendChild(textLi);
  getResList.appendChild(createLi);
